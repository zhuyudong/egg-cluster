...
// fork 子进程或重启 https://github.com/node-modules/cfork
const cfork = require('cfork');
// 为对象注册一个 ready 方法 https://github.com/node-modules/ready
const ready = require('get-ready');
// 检查端口是否被占用 https://github.com/node-modules/detect-port
...
class Master extends EventEmitter {
  ...
  constructor(options) {
    ...
    // agent进程 启动成功后再启动 worker进程 fork app workers after agent started
  ...
  forkAgentWorker() {
    ...
    // 主进程收到来自 agent 进程的消息（./agent_worker.js #30）然后转发出去，交给 #102 forwarding agent' message to messenger
    ...
    // 主进程监听 agent 的退出，agent exit message
  ...
  forkAppWorkers() {
    ...
    cfork({
      ...
      // 生产环境才重启 don't refork in local env
  ...
  onAgentExit(data) {
    ...
    // 移除事件监听，remove all listeners to avoid memory leak
